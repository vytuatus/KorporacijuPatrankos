...
namespace TRexGame.Entities
{
...

    {
        ...
        private KorpPat.TRexRunnerGame _trexRunnerGame;
        ...
        private bool TransitionToCannonGame()
        {
            ...
            _cannonSpawnTime = 0;
            _isCannonGame = true;
            if (_shootingFrequencyLeft > 1f)
            {
                Random random = new Random();
                double val = (random.NextDouble() * (0.3f - 0.05f) + 0.05f);
                _shootingFrequencyLeft -= (float) val;
            }
            if (_shootingFrequencyRight > 1f)
            {
                Random random = new Random();
                double val = (random.NextDouble() * (0.3f - 0.05f) + 0.05f);
                _shootingFrequencyRight -= (float) val;
            }
